...
### （2）对于折叠低支持率的分支，此研究中折叠了BS<10%的基因树分支，同时也对trimal前后的数据进行了比较，但发现结果差异并不大后，选择了未经trimal过滤的数据进行后续分析。说明当处理的结果无法带来显著差异时，其实无法判断处理的必要与否
### (3) 针对GTEE,IH,ILS**三者的贡献占比分析，可以加上一个Pearson相关+LOESS回归分析来衡量基因树的变异程度与GTEE,IH,ILS的相关性

### （4）如何定义冲突分支，如很判断某一个分支的ILS，IH强弱
- 冲突分支定义为溯祖法和串联法相互冲突的分支，然后同时比较冲突分支和一致分支在ILS,GTEE,IH三个方面是否存在显著差异。同时对于冲突分支，衡量其中ILS,IH的强弱占比
- 对于ILS，θ值高于中位数（0.0195）的分支被归类为中等至强ILS，位于前10%（>0.05）的分支被归类为强ILS
- 对于基因流，RI值高于中位数（31.52%）的分支被归类为中等至强基因流，位于前10%（>45.00%）的分支被视为表现出强基因流
