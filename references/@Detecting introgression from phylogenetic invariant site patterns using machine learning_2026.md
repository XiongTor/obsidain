...
#### **研究类群：**
- **模拟数据：** 基于 coalescent 模拟（msprime）生成的四分类群（quartet）系统发育树，包含渐渗（introgression）与不完全谱系分选（ILS）两种场景，涵盖不同的分化时间、有效群体大小、渐渗比例等参数组合。
- **实证数据：** 橡树类群 *Quercus* ser. *Virentes*（7个物种），来自 Eaton et al. (2015) 的 RAD-seq 数据集。

#### **数据特征：**
- 输入数据为 **SNP 位点模式频率（site pattern frequency spectra）**，即 quartet 拓扑下所有可能的核苷酸位点模式（如 ABBA、BABA 等）的计数频率。
- 模拟训练集通过 msprime 生成大量不同参数组合下的 SNP 矩阵，提取位点模式频率作为特征向量。

#### **数据规模：**
- 模拟数据：涵盖广泛的参数空间（分化时间、群体大小、渐渗比例、渐渗方向等），生成足够大的训练集用于神经网络训练。
- 实证数据：*Quercus* ser. *Virentes* 的 RAD-seq 数据，包含数千个 SNP 位点。
...
**1） 核心问题：**
当前系统发育基因组学中检测历史渐渗事件的方法存在两大瓶颈：(1) 网络推断方法（如 SNaQ）计算复杂度高，难以处理大规模基因组数据；(2) 溯祖检验方法（如 ABBA-BABA）虽然能处理大规模 SNP 数据，但仅限于 4-5 个样本的成对比较，无法同时利用所有四分类群信息。如何在保持计算效率的同时，充分利用全基因组 SNP 数据中的多位点模式信息来检测渐渗，是一个亟待解决的问题。
